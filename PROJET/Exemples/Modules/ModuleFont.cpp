...
void affichageChaine(int depth,int fonte) {
  ...
  switch (alignement) {
    case CENTER : { int len = strlen(buffer);
                    ...
                    float dy = 6;
                    glRasterPos3f(pixpos[0]-dx+dcx,pixpos[1]-dy-dcy,-pixpos[2]-dcz); }
                  break;
    case RIGHT  : { int len = strlen(buffer);
                    ...
                    glRasterPos3f(pixpos[0]-dx+dcx,pixpos[1]-dy-dcy,-pixpos[2]-dcz); }
                  break;
    default     : glRasterPos3f(pixpos[0]+dcx,pixpos[1]-dcy,-pixpos[2]-dcz);
                  break; }
  dcx = dcy = dcz = 0.0F;
...
void affichageChaine(int depth,int fonte,char *buffer) {
  int ww = glutGetWindow();
  if ( !mrf[ww] ) {
    mrf[ww] = 1 ;
    makeRasterFont(ww); }
...
void simpleBitmapOutput(int depth,int fonte,char *format,...) {
  ...
  va_end(args);
  affichageChaine(depth,fonte,buffer);
}
...
void simpleBitmapOutput(int fonte,char *format,...) {
  ...
  vsprintf(buffer,format,args);
  va_end(args);
  affichageChaine(0,fonte,buffer);
}
...
void simpleBitmapOutput(char *format,...) {
  ...
  vsprintf(buffer,format,args);
  va_end(args);
  affichageChaine(1,1,buffer);
}

void affichageCaractere(int depth,int fonte,char c) {
  glPixelStorei(GL_UNPACK_ALIGNMENT,1);
  glPushMatrix();
  glMatrixMode(GL_PROJECTION);
  glPushMatrix();
  ...
  switch (fonte) {
    ...
    case 4 : largeurCaractere = 8.0F ;
             break; }    
  switch (alignement) {
    case CENTER : { float dx = largeurCaractere/2;
                    float dy = 6;
                    glRasterPos3f(pixpos[0]-dx+dcx,pixpos[1]-dy-dcy,-pixpos[2]-dcz); }
                  break;
    case RIGHT  : { float dx = largeurCaractere;
                    float dy = 6;
                    glRasterPos3f(pixpos[0]-dx+dcx,pixpos[1]-dy-dcy,-pixpos[2]-dcz); }
                  break;
    default     : glRasterPos3f(pixpos[0]+dcx,pixpos[1]-dcy,-pixpos[2]-dcz);
  ...
  if ( posval ) {
    switch ( fonte ) {
      case 1 : glBitmap(8,13,0.0,2.0,0.0,0.0,regular8x13[c-32]);
               break;
      case 2 : glBitmap(8,10,0.0,2.0,0.0,0.0,regular6x10[c-32]);
               break;
      case 3 : glBitmap(8,13,0.0,2.0,0.0,0.0,symbol[c-32]);
               break;
      case 4 : glBitmap(8,13,0.0,2.0,0.0,0.0,dessin[c-32]);
               break; } }
  glPopAttrib();
  glMatrixMode(GL_PROJECTION);
  glPopMatrix();
...
void simpleBitmapOutput(int depth,int fonte,char c) {
  affichageCaractere(depth,fonte,c);
}

void simpleBitmapOutput(int fonte,char c) {
  affichageCaractere(0,fonte,c);
}

void simpleBitmapOutput(char c) {
  affichageCaractere(0,1,c);
}
...
void lineOutput(int xi,int yi,int xf,int yf) {
  ...
  if ( posval ) {
    glBegin(GL_LINES);
    glVertex3f(pixpos[0]+xi,pixpos[1]-yi,-pixpos[2]); 
    glVertex3f(pixpos[0]+xf,pixpos[1]-yf,-pixpos[2]);
    glEnd(); }
  glPopAttrib();
  glMatrixMode(GL_PROJECTION);
  glPopMatrix();
